...
exst=[exist([SYS.Casename '_arr0.m'],'file')==2,... % Check for Serpent output files
...
SYS.RR(1)=[]; SYS.RR(end+1)=SYS.RR(2);  % Remove oldest step data
...
run([SYS.Casename '_det0.m']); % Get integral fluxes and reaction rates
...
for i=SYS.IDX.MAT.burn % Read burn-up matrix for burnable materials
    MAT(i)=MAT(i).loadBurnMatrix;
end

run([SYS.Casename '_res.m']); % Get Leakage +Serpent k-eff/inf
...
run([SYS.Casename '_arr0.m']); % Get reaction rates
rr(:,[1 4 5 7])=[]; % Remove useless data
rr(rr(:,3)==0,:)=[]; % Remove zero rates
ZAI=unique(rr(:,1));
rr_sorted=zeros(length(ZAI),4);
for i=1:length(ZAI)
    rr_sorted(i,1)=sum(rr(rr(:,1)==ZAI(i)&ismember(rr(:,2),[102:1:117 600:1:849]),3)); % total capture (n,0n)
    rr_sorted(i,2)=sum(rr(rr(:,1)==ZAI(i)&ismember(rr(:,2),[18:1:21 38]),3)); % (n,f)
...
RROidx=~ismember(ZAI,MAT(1).ZAI); %not in MAT vector
SYS.RR(3).notInMat=struct('ZAI',num2cell([-1;ZAI(RROidx)]),...
    'capt',num2cell([0;rr_sorted(RROidx,1)]),...
    'fiss',num2cell([0;rr_sorted(RROidx,2)]),...
    'n2n',num2cell([0;rr_sorted(RROidx,3)]),...
