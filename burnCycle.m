...
if(SYS.PCC.corrector)
  ...
  prefix2='PREDICTOR';
  if(SYS.PCC.active)
    SYS.burnVec2=CRAMsolve(SYS.MTX.total{2},SYS.tStep(end)*OPT.nSteps(SYS.ouCntr)/OPT.nSubSteps,SYS.burnVec);
    for i=1:1:OPT.nSubSteps-1
      SYS.burnVec2=CRAMsolve(SYS.MTX.total{2},SYS.tStep(end)*OPT.nSteps(SYS.ouCntr)/OPT.nSubSteps,SYS.burnVec2);
    end
    SYS.nowTime(end+1)=SYS.nowTime(end)+SYS.tStep(end)*OPT.nSteps(SYS.ouCntr)/(24.0*3600.0); %in EFPD
  else
    SYS.burnVec2=CRAMsolve(SYS.MTX.total{2},SYS.tStep(end)/OPT.nSubSteps,SYS.burnVec);
    for i=1:1:OPT.nSubSteps-1
      SYS.burnVec2=CRAMsolve(SYS.MTX.total{2},SYS.tStep(end)/OPT.nSubSteps,SYS.burnVec2);
    end
